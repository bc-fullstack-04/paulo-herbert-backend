...
public class WalletService {
    ...
    public ResponseWalletDto getByUser() {
        ...
        return new ResponseWalletDto(walletRepository.findByUsers(currentUser).orElseThrow());
    }

    public ResponseWalletDto insertCredit(BigDecimal value) {
        Users currentUser = usersService.getLoggedUser();
        Wallet currentWallet = walletRepository.findByUsers(currentUser).orElseThrow();
        if(value.compareTo(BigDecimal.ZERO) > 0)
            currentWallet.setBalance(currentWallet.getBalance().add(value));
        else
            throw new IllegalArgumentException("Invalid value: "+value);
        return new ResponseWalletDto(this.saveWallet(currentWallet));
    }

    public void debit(WalletOperationDto walletDto) {
        Users users = usersService.findByEmail(walletDto.getUserEmail());
